...
const KIND_ICON: Record<HelplineKind, ReactNode> = {
  fire_station: <Flame className="w-5 h-5 text-red-600" />,
  hospital: <Cross className="w-5 h-5 text-emerald-600" />,
  police: <Shield className="w-5 h-5 text-blue-600" />,
};

const KIND_BORDER: Record<HelplineKind, string> = {
  fire_station: "border-red-200 bg-red-50",
  hospital: "border-emerald-200 bg-emerald-50",
...
export default async function HelplinePage(props: { searchParams: Promise<{ roomId?: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-zinc-900 font-sans p-6 md:p-10 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        <Link
          href="/staff"
          className="inline-flex items-center gap-2 text-zinc-500 hover:text-zinc-900 mb-6 transition-colors"
        >
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            {latestMessage?.summary && (
              <p className="text-zinc-600 italic text-sm mt-2 max-w-2xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          <div className="lg:col-span-3 h-[520px]">
            {hotel ? (
              ...
              <div className="h-full flex items-center justify-center bg-white border border-zinc-200 rounded-2xl text-zinc-500 shadow-sm">
                No hotel configured.
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-2 flex flex-col gap-3 max-h-[520px] overflow-y-auto pr-1">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-zinc-500 font-mono">{helplines.length} found</span>
            </div>

            {helplines.length === 0 && (
              <div className="bg-white border border-zinc-200 rounded-xl p-5 text-sm text-zinc-500 shadow-sm">
                No nearby {helplineLabel(primaryKind).toLowerCase()}s returned. Default to <strong className="text-red-600">112</strong>.
              </div>
            )}
            ...
            {helplines.map((h: Helpline) => (
              <div
                key={h.id}
                className={`rounded-xl p-4 border ${KIND_BORDER[h.kind]} shadow-sm hover:shadow-md transition-shadow`}
              >
                ...
                <h3 className="text-zinc-900 font-semibold text-sm mb-2 leading-tight">{h.name}</h3>
                {h.phone ? (
                  <a
                    href={`tel:${h.phone}`}
                    className="inline-flex items-center gap-2 text-red-600 hover:text-red-700 font-mono text-sm font-bold"
                  >
                    <Phone className="w-3.5 h-3.5" />
                    {h.phone}
                  </a>
                ) : (
                  <span className="text-xs text-zinc-400 italic">No phone listed — use 112</span>
                )}
              </div>
            ))}

            <div className="mt-2 bg-red-50 border border-red-200 rounded-xl p-4 sticky bottom-0 shadow-sm">
              <p className="text-[10px] uppercase font-bold tracking-widest text-red-600 mb-1">
                Always Available (India)
              </p>
              <a
                href="tel:112"
                className="text-2xl font-bold text-red-700 font-mono tracking-wider hover:text-red-800"
              >
                112
              </a>
            </div>
